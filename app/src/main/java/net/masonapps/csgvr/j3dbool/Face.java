...
public class Face implements Cloneable {
    ...
    public double getArea() {
        ...
        Vector3 p2 = v2.getPosition();
        Vector3 p3 = v3.getPosition();
        Vector3 xy = new Vector3(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z).nor();
        Vector3 xz = new Vector3(p3.x - p1.x, p3.y - p1.y, p3.z - p1.z).nor();

        double a = p1.dst(p2);
        double c = p1.dst(p3);
        double B = Math.acos(xy.dot(xz));
    ...
    /**
     * Classifies the face if one of its vertexArray are classified as INSIDE or OUTSIDE
     *
     ...
     */
    private boolean hasPoint(Vector3 point) {
